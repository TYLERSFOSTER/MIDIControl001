...
$$
t_{\text{on}}\;<\;t_{\text{off}}\;\;\in\;\mathbb{R}_{\ge 0}\quad\text{( with $t_{\text{off}}=\infty$ if key held )}
$$
...
$$
A^{\text{env}} : \mathbb{R}^2 \longrightarrow [0,1]\quad\text{( defined in §2.2 )}
$$
...
$$
A^{\text{env}}_{i}:\mathbb{R}\longrightarrow[0,1],
$$
...
Let $K\in\mathbb{N}$ and *margin* $\delta\ge 0$. At time $t$, among $\mathcal{E}_{\text{elig}}(t)$, choose $\mathcal{E}_{\text{act}}(t)$ as the size‑$K$ set with largest $\text{score}_{i}(t)$, preferring incumbents unless a challenger exceeds the $K$-th score by $\delta$. When membership changes, each emitter $i$ carries a *mix gain* $g_i(t)\in[0,1]$ satisfying
$$
\dot g_i(t)=
\begin{cases}
